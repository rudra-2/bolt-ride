...
export const fetchChargingStatus = async (stationId) => {
  try {
    const res = await fetch(`${API_BASE_URL}/${stationId}/charging-status/`);
    ...
    return { status: "error", message: error.message, charging_vehicles: [] };
  }
};
...
export const stopCharging = async (stationId, vehicleId) => {
  try {
    const res = await fetch(`${API_BASE_URL}/${stationId}/stop-charging/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
