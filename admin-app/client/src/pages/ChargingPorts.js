...
export default function ChargingPorts() {
  ...
  const [chargingVehicles, setChargingVehicles] = useState([]);
  ...
  // Auto-refresh charging status every 5 seconds
  useEffect(() => {
    const interval = setInterval(async () => {
      if (stationId) {
        const chargingData = await fetchChargingStatus(stationId);
        if (chargingData.status === "success") {
          setChargingVehicles(chargingData.charging_vehicles || []);
        }
      }
    }, 5000);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const [portsData, vehiclesData, chargingData] = await Promise.all([
        fetchChargingPorts(stationId),
        fetchAvailableVehicles(stationId),
        fetchChargingStatus(stationId)
      ]);
  ...
  const refreshData = async () => {
    ...
    if (chargingData.status === "success") setChargingVehicles(chargingData.charging_vehicles || []);
  };
  ...
  const handleVehicleAssignment = async (vehicleId) => {
    ...
    if (result.status === "success") {
      ...
      alert(`🔋 Vehicle ${vehicleId} assigned to port ${selectedPort.port_id} and charging started!`);
    } else {
  ...
  const handleConfirmRemoveVehicle = async () => {
    ...
    if (result.status === "success") {
      ...
      setSelectedPort(null);
      alert(`✅ Vehicle removed from port ${selectedPort.port_id} and charging stopped`);
    } else {
      alert(result.message || "Failed to remove vehicle");
    }
  };

  const handleStopCharging = async (vehicleId) => {
    const result = await stopCharging(stationId, vehicleId);
    
    if (result.status === "success") {
      await refreshData();
      alert(`⏹️ Charging stopped for vehicle ${vehicleId}`);
    } else {
      alert(result.message || "Failed to stop charging");
    }
  };

  const getVehicleChargingInfo = (vehicleId) => {
    return chargingVehicles.find(cv => cv.vehicle_id === vehicleId);
  };

  const getBatteryColor = (batteryLevel) => {
    if (batteryLevel >= 80) return "text-green-600";
    if (batteryLevel >= 60) return "text-yellow-600";
    if (batteryLevel >= 40) return "text-orange-600";
    return "text-red-600";
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-5 gap-6 mb-8">
          ...
          <div className="bg-white shadow rounded-lg p-6 flex items-center gap-3">
            <div className="w-8 h-8 bg-orange-200 rounded-full flex items-center justify-center">🔋</div>
            <div>
              <p className="text-sm text-gray-500">Currently Charging</p>
              <p className="text-2xl font-bold text-orange-600">{chargingVehicles.length}</p>
            </div>
          </div>
        </div>
          ...
          <div className="mb-8">
            <div className="bg-white shadow rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-800">🔋 Live Charging Status</h2>
                <span className="bg-green-100 text-green-800 px-2 py-1 rounded text-sm">
                  {chargingVehicles.length} vehicles charging
                </span>
              </div>
              
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {chargingVehicles.map(vehicle => (
                  <div key={vehicle.vehicle_id} className="border rounded-lg p-4 bg-gradient-to-r from-green-50 to-blue-50">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-semibold">{vehicle.vehicle_id}</h3>
                      <button
                        onClick={() => handleStopCharging(vehicle.vehicle_id)}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`font-bold text-lg ${getBatteryColor(vehicle.battery_level)}`}>
                          {getBatteryIcon(vehicle.battery_level)} {vehicle.battery_level}%
                        </span>
                      </div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full transition-all duration-500 ${
                            ...
                            vehicle.battery_level >= 60 ? 'bg-yellow-500' :
                            vehicle.battery_level >= 40 ? 'bg-orange-500' : 'bg-red-500'
                          }`}
                      ...
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>Port: {vehicle.charging_port_id}</span>
                        <span>
                          {vehicle.charging_duration_minutes}m
