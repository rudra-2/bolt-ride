          ...
          <div className="space-y-8">
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-2xl shadow-xl p-8">
                ...
                <div className="space-y-4">
                  {report.revenue.payment_status && report.revenue.payment_status.length > 0 ? (
                    report.revenue.payment_status.map((status, index) => (
                      <div key={index} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl hover:shadow-md transition-all">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-gray-800 capitalize">{status._id}</div>
                            <div className="text-sm text-gray-500">{status.count} transactions</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="font-bold text-green-600">₹{status.total_amount.toLocaleString()}</div>
                          <div className="text-sm text-gray-500">
                            {report.revenue.total_revenue > 0 ? ((status.total_amount / report.revenue.total_revenue) * 100).toFixed(1) : 0}%
                          </div>
                        </div>
                      </div>
                    ))
